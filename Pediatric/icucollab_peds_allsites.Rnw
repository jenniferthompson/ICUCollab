...
period (up to 12 months). Code for all data management and analysis can be found
...
quantile.example <- quantile(subset(demog, data.time == 'Baseline')$piculos,
...
Table \ref{table:descstats} describes the patients whose data was collected during the baseline
and implementation data collection periods. There are \Sexpr{format(length(unique(demog$id)), big.mark = ',')} unique patient IDs currently recorded. (In addition,
\Sexpr{length(nomonth.ids)} patients had no ``month" entered in the database and therefore cannot
be counted when comparing the two cohorts.)

For continuous variables (like days of invasive ventilation), the 25th, 50th, and 75th percentiles
...
For example, for PICU length of stay in the baseline cohort (see Table \ref{table:descstats}),
...
Add description of ever on mv, mv among exposed, mv among all

<<desccohort>>=
desc.all <- summaryM(hosp.f.latex + age.f + sex.f + race.combined + english.f +
                       severity.combined + fss.preadm + fss.adm + fss.dc + popc.preadm + popc.adm +
                       popc.dc + ever.vent + ventdays.exp + ventdays + piculos + hosplos +
                       mortality.f ~ data.time,
                     data = demog)
...
pctdays.exersafety <- round((days.exersafety / n.days)*100)
npct.exersafety <- npctString(days.exersafety, n.days)

## How many days had mobility therapy?
days.exer <- mapSum('had.exer')
pctdays.exer <- round((days.exer / n.days)*100)
npct.exer <- npctString(days.exer, n.days)

## How many days had mobility protocol followed?
days.exer.protocol <- mapSum('exer.protocol')
pctdays.exer.protocol <- round((days.exer.protocol / n.days)*100)
npct.exer.protocol <- npctString(days.exer.protocol, n.days)
...
rownames(perf.table) <- c('Days recorded',
                          '\\emph{Days with pain data}',
                          'Days with >=1 asmt with pain >=5',
                          'Assessments with pain >=5',
                          '\\emph{Days with sedation data}',
                          'Days on cont./intermittent sedation',
                          '~~~\\emph{Days with interruption data}',
                          '~~~Days interrupted (SAT)',
                          '~~~\\emph{Days with weaning data}',
                          '~~~Days weaned via protocol',
                          'Days on any sedation (inc. PRN)',
                          '~~~\\emph{Days with deep sedation data}',
                          '~~~Days with deep sedation*',
                          '\\emph{Days with invasive MV data}',
                          'Days on invasive MV',
                          '~~~\\emph{Days with SBT safety data}',
                          '~~~Days SBT safety screen done',
                          '~~~\\emph{Days with SBT data}',
                          '~~~Days SBT done',
                          '~~~Days SBT protocol followed**',
                          '\\emph{Days with anxiolysis data}',
                          'Days received anxiolytics of interest',
                          '~~~Days received benzodiazepines',
                          '~~~Days received ketamine',
                          '~~~Days received diphenhydramine',
                          'Days delirium assessed',
                          '~~~Number of assessments per day',
                          '~~~Days delirium present',
                          'Days mobility safety screen done',
                          'Days mobilization performed',
                          'Days mobility protocol followed**',
                          '\\emph{Family presence data available}',
                          'Days family or caregiver present',
                          '\\emph{Family intervention data available}',
                          'Days family participated in nonpharm inter.')
@
...
latex(perf.table, file = '',
      ...
      rgroup = c('', 'A: Assess, Prevent, Manage pain', 'B: Both SAT and SBT - Sedation',
                 'B: Both SAT and SBT - ERT/Breathing', 'C: Choice of Analgesia and Sedation',
                 'D: Delirium - Assess, Prevent, Manage***', 'E: Exercise/Early Mobility',
                 'F: Family Engagement/Empowerment'),
      n.rgroup = c(1, 3, 9, 7, 5, 3, 3, 4),
      insert.bottom = "* Deep sedation refers to SBS $\\le$ -2 or RASS $\\le$ -4; ** Protocol = safety screen performed and, if passed, SBT/mobilization performed; *** Delirium present if highest CAPD $\\ge$9 or present on psCAM, pCAM or CAM-ICU; considered to be assessed if one or more of those assessment tools is available")
