...
Please see Section \ref{sec:otherdx} for a complete listing of descriptions for ``other" diagnoses.
...
demog_categoricals <- demog %>%
  dplyr::select(id, data.time, age.f, sex.f, race.combined, #english.f,
                severity.combined) %>%
  gather(key = char_type, value = char_value, age.f:severity.combined) %>%
  ...
  mutate(prop_value = (n_value / n_total)*100,
         ...
         char_type = factor(ifelse(char_type == "age.f", 1,
...
demog_admitdx <- demog %>%
  ...
  gather(key = diagnosis, value = percent, dx.surgery:dx.other) %>%
...
demog_continuous <- demog %>%
  dplyr::select(id, data.time, prism3, pim2, fss.preadm, fss.adm,
                popc.preadm, popc.adm) %>%
  gather(key = cont_type, value = cont_value, prism3:popc.adm) %>%
  filter(!is.na(data.time) & !is.na(cont_value)) %>%
  mutate(cont_type = factor(ifelse(cont_type == "prism3", 1,
                            ifelse(cont_type == "pim2", 2,
                            ifelse(cont_type == "fss.preadm", 3,
                            ifelse(cont_type == "fss.adm", 4,
                            ifelse(cont_type == "popc.preadm", 5,
                            ifelse(cont_type == "popc.adm", 6, NA)))))),
                            levels = 1:6,
                            labels = c("PRISM III",
                                       "PIM 2",
                                       "FSS, pre-admission",
...
<<vis_demog_cat, results = 'asis', fig.width=7.5, fig.height=7, fig.align='center'>>=
plot.cat <- ggplot(data = demog_categoricals,
                   aes(x = category, y = prop_value)) +
  facet_wrap(~ char_type, ncol = 1, scales = 'free_y') +
  geom_bar(aes(group = data.time, fill = data.time),
           stat = 'identity', position = 'dodge') +
  scale_fill_manual(values = c(as.character(peds.colors["A"]),
                               as.character(peds.colors["D"]))) +
...
plot.admitdx <- ggplot(data = demog_admitdx,
                       aes(x = diagnosis.f, y = percent)) +
  facet_wrap(~ facet.text, ncol = 1) +
  geom_bar(aes(group = data.time, fill = data.time),
           stat = 'identity', position = 'dodge') +
  scale_fill_manual(values = c(as.character(peds.colors["A"]),
                               as.character(peds.colors["D"]))) +
...
multiplot(plot.cat, plot.admitdx, layout = matrix(c(1, 1, 1, 2, 2), nrow = 1))

@

<<vis_demog_cont, results = 'asis', fig.width=7.5, fig.height=3.25, fig.align='center'>>=
ggplot(data = demog_continuous, aes(x = data.time, y = cont_value)) +
  facet_wrap(~ cont_type, scales = 'free_y') +
  # geom_point(aes(colour = data.time),
  #            position = position_jitter(), alpha = 0.15) +
  geom_boxplot(aes(colour = data.time, fill = data.time),
               alpha = 0.5, outlier.size = 0.5) +
  scale_colour_manual(values = c(as.character(peds.colors["D"]),
                                 as.character(peds.colors["A"]))) +
  scale_fill_manual(values = c(as.character(peds.colors["D"]),
                               as.character(peds.colors["A"]))) +
...
<<vis_outcome_prep>>=
outcome_cat <- demog %>%
  dplyr::select(id, data.time, ever.vent, mortality.f) %>%
  gather(key = char_type, value = char_value, ever.vent:mortality.f) %>%
  group_by(data.time, char_type, char_value) %>%
  summarise(n_value = n()) %>%
  ungroup() %>%
  left_join(time_totals, by = "data.time") %>%
  mutate(prop_value = (n_value / n_total)*100,
         data.time = factor(ifelse(data.time == "Implementation", 1, 2),
                            levels = 1:2,
                            labels = c("Implementation", "Baseline")),
         char_type = factor(ifelse(char_type == "ever.vent", 1,
                            ifelse(char_type == "mortality.f", 2, NA)),
                            levels = 1:2,
                            labels = c("Ever on MV", "Died in hospital")),
         ## Manually create factor variables to get categories in right order
         category = factor(ifelse(char_value == "Never on MV", 3,
                           ifelse(char_value == ">=1 day on MV", 2,
                           ifelse(char_value == "Unknown (no data available)",
                                  1,
                           ifelse(char_value == "Discharged alive", 5,
                           ifelse(char_value == "Died in hospital", 4,
                                  NA))))),
                           levels = 1:5,
                           labels = c("Unknown\n(no data available)",
                                      ">=1 day on MV",
...
plot.outcomes.cat <- ggplot(data = outcome_cat,
                            aes(x = category, y = prop_value)) +
  facet_wrap(~ char_type, ncol = 2, scales = 'free_y') +
  geom_bar(aes(group = data.time, fill = data.time),
           stat = 'identity', position = 'dodge') +
  scale_fill_manual(values = c(as.character(peds.colors["A"]),
                               as.character(peds.colors["D"]))) +
...
outcomes_continuous <- demog %>%
  dplyr::select(id, data.time, fss.dc, popc.dc, ventdays, piculos, hosplos) %>%
  gather(key = cont_type, value = cont_value, fss.dc:hosplos) %>%
  filter(!is.na(data.time) & !is.na(cont_value)) %>%
  mutate(cont_type = factor(ifelse(cont_type == "fss.dc", 1,
                            ifelse(cont_type == "popc.dc", 2,
                            ifelse(cont_type == "ventdays", 3,
                            ifelse(cont_type == "piculos", 4,
                            ifelse(cont_type == "hosplos", 5, NA))))),
                            levels = 1:5,
                            labels = c("FSS, discharge",
                                       "POPC, discharge",
                                       "Days on MV",
                                       "PICU length of stay",
                                       "Hospital length of stay")))

plot.outcomes.cont <- ggplot(data = outcomes_continuous,
                             aes(x = data.time, y = cont_value)) +
  facet_wrap(~ cont_type, scales = 'free_y') +
  # geom_point(aes(colour = data.time),
  #            position = position_jitter(), alpha = 0.15) +
  geom_boxplot(aes(colour = data.time, fill = data.time),
...
<<vis_outcome, results = 'asis', fig.width = 7.5, fig.height = 6.5, fig.align = 'center'>>=
multiplot(plot.outcomes.cat, plot.outcomes.cont,
...
Tables \ref{table:benzob6}-\ref{table:opioidb12} summarize drug doses
\textbf{per 24 hours} for benzodiazepines and opioids.
...
If at least one medication in a category (benzodiazepines or opioids) had a dose recorded (including zero), all NA values in that category were assumed to be zero. Otherwise, doses were considered missing and are not included in the following tables. For example, if a patient-day had a dose recorded for fentanyl but nothing recorded for morphine or hydromorphone, both morphine and hydromorphone were assumed to have doses of 0 for that day. If, on the other hand, none of the three opioids of interest were recorded, no opioid data for that day is described - doses are left as missing and that day is not included in Tables \ref{table:opioidb6}-\ref{table:opioidb12}.
...
meds.table <- summaryM(has.benzo + benzo.total + benzo.total.eo + loraz.scaled +
                         loraz.scaled.eo + midaz.scaled + midaz.scaled.eo +
                         has.opioid + opioid.total + opioid.total.eo +
                         morph.scaled + morph.scaled.eo + hydromorph.scaled +
                         hydromorph.scaled.eo + fent.scaled +
                         fent.scaled.eo ~ month.cat,
                       data = compliance)

benzo.table.b6 <- summaryM(benzo.rcvd + benzo.total + benzo.total.eo +
                             loraz.rcvd + loraz.scaled + loraz.scaled.eo +
                             midaz.rcvd + midaz.scaled +
                             midaz.scaled.eo ~ month.cat,
                           data = subset(compliance,
                                         month.cat %in% c('Baseline',
                                                         paste('Imp.', 1:6)) &
                                         has.benzo))

benzo.table.b12 <- summaryM(benzo.rcvd + benzo.total + benzo.total.eo +
                              loraz.rcvd + loraz.scaled + loraz.scaled.eo +
                              midaz.rcvd + midaz.scaled +
                              midaz.scaled.eo ~ month.cat,
                            data = subset(compliance,
...
opioid.table.b6 <- summaryM(opioid.rcvd + opioid.total + opioid.total.eo +
                              morph.rcvd + morph.scaled + morph.scaled.eo +
                              hydromorph.rcvd + hydromorph.scaled +
                              hydromorph.scaled.eo + fent.rcvd +
                              fent.scaled + fent.scaled.eo ~ month.cat,
                            data = subset(compliance,
                                          month.cat %in% c('Baseline',
                                                          paste('Imp.', 1:6)) &
                                          has.opioid))

opioid.table.b12 <- summaryM(opioid.rcvd + opioid.total + opioid.total.eo +
                               morph.rcvd + morph.scaled + morph.scaled.eo +
                               hydromorph.rcvd + hydromorph.scaled +
                               hydromorph.scaled.eo + fent.rcvd +
                               fent.scaled + fent.scaled.eo ~ month.cat,
...
meds.table.b6 <- summaryM(has.benzo + benzo.total + benzo.total.eo +
                            loraz.scaled + loraz.scaled.eo + midaz.scaled +
                            midaz.scaled.eo + has.opioid + opioid.total +
                            opioid.total.eo + morph.scaled + morph.scaled.eo +
                            hydromorph.scaled + hydromorph.scaled.eo +
                            fent.scaled + fent.scaled.eo ~ month.cat,
                          data = subset(compliance,
                                        month.cat %in% c('Baseline',
                                                         paste('Imp.', 1:6))))

meds.table.b12 <- summaryM(has.benzo + benzo.total + benzo.total.eo +
                             loraz.scaled + loraz.scaled.eo + midaz.scaled +
                             midaz.scaled.eo + has.opioid + opioid.total +
                             opioid.total.eo + morph.scaled + morph.scaled.eo +
                             hydromorph.scaled + hydromorph.scaled.eo +
...
<<printbenzotables, results = 'asis'>>=
latex(benzo.table.b6, file = '',
      title = '',
      caption = 'Daily Benzodiazepine Use, Baseline - Implementation Month 6. Doses are summarized by {\\scriptsize 25th} 50th {\\scriptsize 75th} percentiles. Patient-days are included if at least one value, including 0, was recorded for lorazepam and/or midazolam',
      digits = 2,
      prmsd = FALSE,
      prn = FALSE,
      prN = FALSE,
...
latex(benzo.table.b12, file = '',
      title = '',
      caption = 'Daily Benzodiazepine Use, Baseline + Implementation Months 7-12. Doses are summarized by {\\scriptsize 25th} 50th {\\scriptsize 75th} percentiles. Patient-days are included if at least one value, including 0, was recorded for lorazepam and/or midazolam',
      digits = 2,
      prmsd = FALSE,
      prn = FALSE,
      prN = FALSE,
...
\clearpage
\subsection{Opioids}
<<printopioidtables, results = 'asis'>>=
latex(opioid.table.b6, file = '',
      title = '',
      caption = 'Daily Opioid Use, Baseline - Implementation Month 6. Doses are summarized by {\\scriptsize 25th} 50th {\\scriptsize 75th} percentiles. Patient-days are included if at least one value, including 0, was recorded for morphine, fentanyl and/or hydromorphone',
      digits = 2,
      prmsd = FALSE,
      prn = FALSE,
      prN = FALSE,
      ...
      long = TRUE,
      where = '!h',
      label = 'table:opioidb6',
      size = 'scriptsize')

latex(opioid.table.b12, file = '',
      title = '',
      caption = 'Daily Opioid Use, Baseline + Implementation Months 7-12. Doses are summarized by {\\scriptsize 25th} 50th {\\scriptsize 75th} percentiles. Patient-days are included if at least one value, including 0, was recorded for morphine, fentanyl and/or hydromorphone',
      digits = 2,
      prmsd = FALSE,
      prn = FALSE,
      prN = FALSE,
...
\section{Descriptions of ``Other" Diagnoses} \label{sec:otherdx}
These descriptions are taken verbatim from the REDCap database.
