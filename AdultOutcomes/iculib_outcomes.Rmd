...
html(summaryM(ever_invas_mv + hrs_invas_mv + ever_noninvas_mv +
                ...
                perf_yn_prop + hosp_los + home_posticu + mob_posticu +
...
inicu_results <- list(
  "mv_results", "res_results", "coma_results", "del_results"
)

## Raw data to add to plots
inicu_outcomevars <- list("on_mv_icu_f_tmw", "on_restraints_icu_tmw",
  ...
  mutate(
    ...
    outcome = factor(
      case_when(
        ...
        outcome == "on_restraints_icu_tmw" ~ 2,
        outcome == "coma_icu_tmw"          ~ 3,
        TRUE                               ~ 4
      ),
      levels = 1:4,
      labels = c("Mechanical Ventilation", "Physical Restraints",
                 "Coma", "Delirium")
    )
  ) %>%
...
inicu_probs <- map_df(inicu_results, get_perf_probs) %>%
  mutate(
    ...
    outcome_label = factor(
      case_when(
        outcome == "Mechanical Ventilation" ~ 4,
        outcome == "Physical Restraints"    ~ 1,
        outcome == "Coma"                   ~ 2,
        TRUE                                ~ 3
      ),
      levels = 1:4,
      labels = c("A: Physical Restraints", "B: Coma",
                 "C: Delirium", "D: Mechanical Ventilation")
    )
  )

inicu_plot_all <- ggplot(data = inicu_probs, aes(x = prop_done, y = yhat)) +
  ...
  geom_point(aes(x = prop_done, y = yhat),
             colour = "gray35", shape = 1, alpha = 0.02, size = 0.25,
             position = position_jitter(height = 0.05, width = 0.025),
             data = inicu_rawdata) +
  scale_x_continuous(name = "Percent of Eligible Bundle Elements Performed",
                     limits = 0:1,
                     breaks = seq(0, 1, 0.2),
                     labels = paste0(seq(0, 1, 0.2) * 100, "%")) +
...
pdf(file = "pubfigures/inicu_plot_all.pdf", height = 4.5, width = 6)
inicu_plot_all
dev.off()
...
versions for each outcome individually.

```{r fig_inicu_three}
inicu_plot_three <-
  ggplot(data = inicu_probs %>% filter(outcome != "Mechanical Ventilation"),
...
inicu_plot_three_horiz <-
  inicu_plot_three + facet_wrap(~ outcome_label, nrow = 1)
inicu_plot_three_vert <-
  inicu_plot_three + facet_wrap(~ outcome_label, ncol = 1)

pdf(file = "pubfigures/inicu_plot_three_horiz.pdf", height = 3, width = 6.5)
inicu_plot_three_horiz
dev.off()

pdf(file = "pubfigures/inicu_plot_three_vert.pdf", height = 6, width = 4.5)
inicu_plot_three_vert
dev.off()
...
pwalk(
  list(
    ...
    "out_title" = c("mv", "res", "coma", "del"),
    "ybreaks" = c(0.1, 0.05, 0.05, 0.05)
  ),
  inicu_singleplot
)
...
tte_plot_three <- ggplot(data = tte_hrs, aes(x = ylabel, y = effect)) +
  ...
  scale_x_discrete(name = "Proportion of Bundle Elements Performed") +
  coord_flip() +
  theme_minimal() +
...
tte_plot_three_horiz <- tte_plot_three + facet_wrap(~ outcome_label, nrow = 1)
tte_plot_three_vert <- tte_plot_three + facet_wrap(~ outcome_label, ncol = 1)

pdf(file = "pubfigures/tte_plot_three_horiz.pdf", height = 3, width = 6.5)
...
pdf(file = "pubfigures/tte_plot_three_vert.pdf", height = 6, width = 4.5)
tte_plot_three_vert +
  scale_y_continuous(name = "Adjusted Hazard Ratio (95% Confidence Interval)",
                     limits = c(0, 2.05),
                     breaks = seq(0, 10, 0.2))
...
pdf(file = "pubfigures/icusummary_plot_all_horiz.pdf", height = 3.5, width = 6)
icusummary_plot_all + facet_wrap(~ outcome, nrow = 1)
dev.off()

pdf(file = "pubfigures/icusummary_plot_all_vert.pdf", height = 6, width = 4)
icusummary_plot_all + facet_wrap(~ outcome, ncol = 1)
dev.off()
