...
demog$month_cat <- with(demog, {
  factor(ifelse(month_f %in% paste("Month", 1:6), 1,
         ...
         levels = 1:15,
         labels = c("Baseline", paste("Implementation", 1:14)))
})
...
# Additional Variable Calculation Information
...
```{r print_descbase}
html(summaryM(hosp_f + month_cat + age_f + sex_f + race_cat + hisp_cat + bmi +
                home_preadmit + mob_preadmit + primary_admit + icu_type_comb +
                any_soi + apacheii_score + apacheiii_score + apacheiv_score +
                sofa_score ~ 1,
              data = demog_desc,
              continuous = 5),
     what = "%",
