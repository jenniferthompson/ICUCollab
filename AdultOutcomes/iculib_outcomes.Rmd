...
```{r print_descbase}
html(summaryM(hosp_f + hosp_type + month_cat + age_f + sex_f + race_cat +
...
Further details are included below for reference. `FALSE` (in column headers) / black dots (in figures) indicate at least some missing data, while `TRUE` / white dots indicate complete data.

```{r logmods_compare_missing_prep}
## -- Create indicators for missingness among outcomes (with/without ----------- ## -- delirium), covariates, overall (with/without delirium) -------------------
...
```{r logmods_compare_missing_incdel, fig.height = 7, fig.width = 7}
...
```{r logmods_compare_missing_notincdel, fig.height = 7, fig.width = 7}
