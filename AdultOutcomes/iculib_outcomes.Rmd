...
miss_var_coma <- miss_var_summary(subset(df_logmod_coma, !include)) %>%
  filter(percent > 0 & variable %in% coma_vars)
...
miss_var_del <- miss_var_summary(subset(df_logmod_del, !include)) %>%
  filter(percent > 0 & variable %in% del_vars)
...
miss_var_res <- miss_var_summary(subset(df_logmod_res, !include)) %>%
  filter(percent > 0 & variable %in% res_vars)
...
miss_var_pain <- miss_var_summary(subset(df_logmod_pain, !include)) %>%
  filter(percent > 0 & variable %in% pain_vars)
...
Due to the diversity of typical data collection across our `r length(unique(demog$hosp))` sites nationwide, we are unable to adjust for
measures of severity of illness, which could be a major unmeasured confounder.
(Please see Table 1 for further details.) A traditional sensitivity analysis
could actually *introduce* bias: the most common SOI score was only collected at
six sites, and there is a strong chance that those six sites have additional
unmeasured differences from the other sites (for example, nearly all these
scores are from teaching vs community hospitals).

We introduce a "tipping point" analysis to determine how large the relationship between severity of illness and each outcome would need to be in order to tip
each of our "significant" outcomes to inconclusivity, where the confidence
interval for an odds or hazard ratio would include 1. We do this for outcomes
which reach statistical significance in our original analyses for the binary
exposure of whether the entire Bundle was performed (yes/no). This is a
**conservative** analysis; it assumes that severity of illness is not associated
with any of the covariates we were able to include in our models, which is
almost certainly not the case. If there is a relationship between SOI and any of
the covariates, the tipping points would be proportionally larger than shown
below.

We use the `r sum(!is.na(demog$apacheiii_score))` patients with APACHE III, our
most commonly available measure of SOI, available as an estimate of the relationships between SOI and ABCDEF Bundle performance. In case this subset is different from our entire cohort, we also provide a more conservative estimate, where the difference in SOI between exposed (complete performance) and unexposed (incomplete or no performance) groups is 25% larger.

For each outcome, we present the observed relationship between ABCDEF Bundle
performance (yes/no) and the outcome, adjusted for covariates listed above; the
observed difference in standardized means between the exposed (complete
performance) and unexposed (incomplete or no performance) groups; and the
estimate of the odds or hazard ratio that would have to be observed between a
one-unit change in APACHE III and each outcome in order to make our results
inconclusive.
...
tip_args <- list(
  results_list = list(
    mv_results, coma_results, del_results, res_results, iculos_results,
    hosplos_results, death_results
  ),
  outcome_string = c("MV", "Coma", "Delirium", "Physical Restraints",
                     "ICU Discharge", "Hospital Discharge", "Death"),
  model_df = list(
    df_logmod_mv, df_logmod_coma, df_logmod_del, df_logmod_res,
    iculos_dflist[[2]], hosplos_dflist[[2]], death_dflist[[2]]
  )
)
...
As one example, given both our original results and the observed relationship in
our data between APACHE III score and ABCDEF Bundle performance, there would have to be an odds ratio of
`r tipping_points %>% filter(outcome == "MV") %>% pull(tip_obs) %>% round(2)`
for a one-unit change in APACHE III vs mechanical ventilation in order to move
our observed results to inconclusivity (confidence interval including 1). With a
more conservative approach, we would still need an odds ratio of `r tipping_points %>% filter(outcome == "MV") %>% pull(tip_cons) %>% round(2)` to
tip the scales. These are pretty extreme and unlikely values (especially given
the inherent conservatism in this approach, assuming that SOI is unrelated to
any other covariates); thus we have more confidence that even if the true
adjusted association between Bundle performance and MV is smaller than observed
after adjusting for SOI, it would still be meaningful.

For our outcomes of ICU and hospital discharge, however, where our original
results are much closer to inconclusivity, it would take a much smaller
relationship between SOI and each outcome to change our results.

```{r print_tippt}
tipping_points %>%
  kable(format = "html",
        col.names = c("Outcome", "Original Results", "Observed SMD",
                      "Observed", "Conservative"),
        align = c("l", rep("r", 4)),
        digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  add_header_above(c(" " = 3, "Tipping Point" = 2))
