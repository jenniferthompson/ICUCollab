...
We will use a Cox proportional hazards model with time-dependent covariates to assess the relationship between ABCDEF bundle compliance/performance on a given day and the likelihood of being discharged from the ICU at any point within 7 days. Patients who die prior to ICU discharge will be censored at the time of death. Outcomes are restricted to 7 days because at most sites, data collection on ABCDEF bundle elements stopped on ICU day 7.
...
## -- Wrapper function for a specified outcome: --------------------------------
## 1. Create time-varying dataset (need to add this still)
## 2. Fit CPH model (returned as "modobj")
## 3. Get df of hazard ratios + p-value (returned as "ors")
## <predicted probabilities don't really apply to Cox models>
fit_results_cph <- function(
  ## Time-varying components and dataset
  exposure = c("comp_yn_f", "comp_prop", "perf_yn_f", "perf_prop"),
  daily_df = compliance,
  ## Single-value variables and dataset (time, status)
  time_var, status_var, time_df = demog,
  ## Model covariates
  covars = c(logmods_covars_base, logmods_covars_daily)
){
  
  ## Add arguments, functionality to create time-varying dataset
  df_tv <- create_tte_data(
    time_var = time_var,
  ...
  return(list("mod" = mod, "hrs" = hrs))
}

## -- Wrapper to do all exposures for a given outcome --------------------------
outcome_results_cph <- function(exposure = c("comp_yn_f", "comp_prop",
                                             "perf_yn_f", "perf_prop"),
                                daily_df = compliance,
                                time_var, status_var, time_df = demog,
                                covars = c(logmods_covars_base,
...
tte_hr_results <- function(
  ...
  outcome_name = c("ICU discharge", "hospital discharge", "ICU death")
){
...
demog <- demog %>%
  mutate(
    ...
    icu_los_edit = ifelse(icu_los >= last_icu_day, icu_los, last_icu_day),
    hosp_los_edit = ifelse(hosp_los >= last_icu_day, hosp_los, last_icu_day),
    ...
    hosp_los_7 = ifelse(hosp_los_edit > 7, 8, hosp_los_edit),
    hosp_dc_7 = factor(
      as.numeric(
    ...
    ## Time to ICU death = same as time to ICU discharge; only indicator changes
    icu_death_7 = factor(
      as.numeric(
        ifelse(is.na(dc_status_f) | is.na(icu_los), NA,
               dc_status_f ==
...
```{r iculos_mods}
iculos_results <-
  outcome_results_cph(time_var = "icu_los_7", status_var = "icu_dc_7")

iculos_plot <- tte_hr_results(iculos_results, outcome_name = "ICU discharge")

## Need to add outcome_info argument, details on missingness

# tmp_df <- create_tte_data(
#   time_var = "icu_los_7",
#   status_var = "icu_dc_7",
#   exposure = "comp_yn_f"
# )
# 
# tmp_mod <- fit_daily_cph(
#     exposure = "comp_yn_f",
#     covars = c(logmods_covars_base, logmods_covars_daily),
#     df = tmp_df
# )
#   
# ## daily_outexp_ors works for hazard ratios too
# tmp_hrs <- daily_outexp_ors(
#   modobj = tmp_mod, exposure = "comp_yn_f", df = tmp_df
# )

# tmp <- fit_results_cph(
#   ## Time-varying components and dataset
#   exposure = c("comp_yn_f"),
#   ## Single-value variables and dataset (time, status)
#   time_var = "icu_los_7", status_var = "icu_dc_7"
# )
