...
create_tte_data <- function(time_var, status_var, exposure){
  use_demog <- demog %>%
    dplyr::select(id,
...
fit_results_cph <- function(exposure = c("comp_yn_f", "comp_prop",
  ...
  mod <- fit_daily_cph(
    exposure = exposure, covars = covars, df = df
  )
  
  ## daily_outexp_ors works for hazard ratios too
  hrs <- daily_outexp_ors(mod, exposure, df)
...
```{r dataprep_iculos}
## Use create_tte_data() mapping over all four exposures to get data sets
