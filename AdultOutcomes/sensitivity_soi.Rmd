...
logmods_covars_base <- c("age_cat_mod", "sex_f", "race_cat_mod", "hisp_cat_mod",
                         ...
                         "primary_admit", "icu_type", "apacheiii_score")
...
**Exceptions to methodology from original analysis:**
...
analysis.
1. Some categorical covariates were collapsed from their original level of
detail due to low counts in some categories. These include age, race, primary
admission reason, and ICU type.
...
demog <- left_join(demog, last_days, by = "id") %>%
  ...
  ## Collapse categorical variables due to small N (math issues otherwise)
  mutate(
    icu_type_comb = fct_collapse(
      icu_type,
      "Mixed medical/surgical" = c(
        "Adult Critical Care Unit", "Adult ICU", "Critical Care Unit", "ICU",
        "Medical & Surgical ICU", "Trauma and Life Support Center"
      ),
      "Surgical/trauma/cardiac" = c(
        "Adult Surgical Heart Unit", "Cardiac Surgical ICU",
        "Cardiothoracic ICU", "Cardiovascular ICU", "Surgical ICU", "Trauma ICU"
      )
    ),
    hosp_comb = fct_lump(hosp_f, prop = 0.20, other_level = "Other sites"),
    race_comb = fct_lump(race_cat_mod, prop = 0.15, other_level = "Other race"),
    age_comb = fct_collapse(
      age_cat_mod,
      "<40" = c("<=29", "30-39")
    ),
    primary_admit_comb = fct_lump(
      primary_admit, prop = 0.05, other_level = "Other reason"
    )
  )

## -- Reset baseline covariates to use collapsed versions ----------------------
logmods_covars_base <- c("age_comb", "sex_f", "race_comb", "hisp_cat_mod",
                         "bmi", "mob_preadmit", "home_preadmit", # "hosp_type",
                         "primary_admit_comb", "icu_type_comb",
                         "apacheiii_score")
...
base_vars <- c("age_comb", "sex_f", "race_comb", "hisp_cat_mod", "bmi",
               "mob_preadmit", "primary_admit_comb", "home_preadmit",
               "icu_type_comb", "hosp_comb", "apacheiii_score")
...
rcs_vars <- c(
  "icu_los_edit",
  ## Not enough variability to use splines here
  # "rcvd_benzo_icu_prop", "rcvd_opioid_icu_prop", "rcvd_propofol_icu_prop",
  # "rcvd_dex_icu_prop", "rcvd_antipsyc_icu_prop",
  # "comfort_care_icu_prop",
  # "on_mv_icu_prop", "coma_icu_prop",
  "bmi", "apacheiii_score",
  # "comp_prop", "perf_prop",
...
more/full performance of the ABCDEF Bundle on a given day is associated with a
lower likelihood of being delirious the following day among the patients we were
able to include in our model.
...
After adjusting for available confounders including APACHE III, full and more
performance of the ABCDEF Bundle on a given day are both associated with higher
odds of significant pain the following day among the patients we were able to
include in our model. This could be due to more proactive assessments being done
in these patients.
...
#      file = "AnalysisData/tte_datasets_apache.Rdata")

load("AnalysisData/tte_datasets_apache.Rdata")
...
After restricting our patient population and adjusting for potential confounders
including APACHE III, more compliance and performance of the ABCDEF Bundle on a
given day are associated with a higher likelihood of ICU discharge; there is a dose-response relationship, such that more compliance and more performance are associated with higher likelihood of ICU discharge.

It is important to note that because most Collaborative sites were only able to collect one week of data, these results only reflect ICU discharges within the first 7 days after ICU admission. ABCDEF Bundle compliance and performance may have different associations with ICU discharge when long ICU stays are able to
be included.
...
There were too few ICU readmissions in our restricted population to allow reliable adjustment for our entire covariate list (only `r sum(df_icureadm$ever_readmit_icu == ">=1 readmission", na.rm = TRUE)`). Since we
would not be able to determine what potential difference was due to adjustment
for APACHE III and what was due to lack of adjustment for other covariates, we
did not include this outcome in our sensitivity analyses.
...
After restricting our patient population and adjusting for potential confounders
including APACHE III, more compliance and performance of the ABCDEF Bundle on a
given day are not strongly associated with a higher likelihood of hospital discharge, though directionally performance does seem to be potentially beneficial. This lack of significance could be due to lack of power (our
restricted sample size may not be sufficient to adjust for all covariates) or a
true confounding effect from severity of illness.

It is important to note that because most Collaborative sites were only able to collect one week of data, these results only reflect hospital discharges within
the first 7 days after ICU admission. ABCDEF Bundle compliance and performance may have different associations with hospital discharge when longer
hospitalizations are able to be included.
...
We did not observe enough deaths in this subset within 7 days of ICU admission
(only `r sum(demog$dc_alive == "Died in hospital", na.rm = TRUE)`) to
sufficiently adjust for all covariates included in the original analyses, much
less adjust for APACHE III. Therefore, we did not include this outcome in the
...
After restricting our population and adjusting for APACHE III, we see no
significant relationship between ABCDEF Bundle performance and discharge to a
location other than home among patients who survived hospitalization.
