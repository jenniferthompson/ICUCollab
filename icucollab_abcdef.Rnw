...
ggplot.prop <-
  function(var,                                             ## X variable to plot
           ...
           title.string = var,                              ## plot title; defaults to X variable name
  ...
  names(crosstab.data) <- gsub(facet.var, 'facetvar',
                               gsub(var, 'proplevel', tolower(names(crosstab.data))))
  ...
  prop.plot <- ggplot(data = crosstab.data, aes(x = proplevel, y = prop, fill = data.time)) +
    ...
    scale_x_discrete(name = use.xlab) +
    scale_y_continuous(name = 'Percent of ICU Days',
                       limits = c(0, 1),
                       breaks = seq(0, 1, 0.25),
    ...
    theme(legend.position = 'bottom',
          ...
          plot.title = element_text(hjust = title.hjust, face = 'bold'),
