...
if(afterlastmonth.ids > 0){
  afterlastmonth.sentence <- paste0("; ", afterlastmonth.ids, " patients had data collected after month ", last.month, " which will be included in a later report")
} else{
  afterlastmonth.sentence <- ""
}
...
compliance <- compliance %>%
  ...
  mutate(## Denominator for all following variables: days in ICU full 24h
         ...
         ## Compliant on SAT: on sedation + SAT screen passed + SAT performed; must have enough
         ## info present to be confirmed compliant
         comp.b.sat = ifelse(!on.sedative.icu, NA,
                      ifelse(!is.na(satscreen.f) &
                               ## If safety screen failed, SAT performance must be missing or
                               ## something other than Yes
                               ((satscreen.f == 'Failed' &
                                  (is.na(satperformed.f) | !(satperformed.f == 'Yes'))) |
                                ## If safety screen passed, SAT must have been performed
                                (satscreen.f == 'Passed' &
                                  !is.na(satperformed.f) & satperformed.f == 'Yes')), TRUE, FALSE)),
...
ref.line.shade <- as.character(magglass.colors['f'])

plot.compliance <- function(compvar,
                            ytitle,
                            ylims = c(0, 1),
                            ybreaks = 0.2,
                            put.legend = c('bottom', 'top'),
                            refline.size = 1.5){
  ...
  ggplot(data = comp.summary,
    ...
    scale_colour_manual(values = as.character(magglass.colors[c(6, 1, 3)]), name = '') +
    scale_alpha_manual(values = c(1, 0.75, 0.75), guide = 'none') +
    theme(legend.position = c(0.98, legend.y),
...
plot.comp.f <- plot.compliance('pct.comp.f',
  ...
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.bundle <-
  plot.compliance('pct.comp.bundle',
                  'Overall ABCDEF Bundle Compliance',
                  ylims = c(0,
                            max(comp.summary[,c('pct.comp.bundle', 'pct.perf.bundle')],
                                na.rm = TRUE)),
                  ybreaks = 0.05) +
  labs(title = 'ABCDEF Bundle Compliance')
plot.perf.bundle <-
  plot.compliance('pct.perf.bundle',
...
plot.comp.elements <- plot.compliance('pct.elem.comp',
                                      ...
                                      ylims = c(0,
                                                max(comp.summary[,c('pct.elem.comp',
                                                                    'pct.elem.perf')],
                                                    na.rm = TRUE)),
                                      ybreaks = 0.05) +
...
plot.assessments <- function(asmtvar){
  ggplot(data = subset(compliance, comp.tracked & !is.na(data.month.short)),
    ...
    geom_boxplot(fill = as.character(magglass.colors['f']),
                 colour = as.character(magglass.colors['f']),
                 outlier.shape = NA,
                 # outlier.size = 0.5,
                 alpha = 0.4) +
    scale_x_discrete(name = '') +
...
plot.compliance.region <- function(compvar,
  ...
  ggplot(data = comp.summary.region.plot,
    ...
    scale_y_continuous(name = ytitle,
                       ...
                       breaks = seq(0, 1, ybreaks),
                       labels = paste0(seq(0, 1, ybreaks)*100, '%')) +
    scale_colour_manual(values = as.character(magglass.colors[c(6, 1, 3)]), guide = 'none') +
...
plot.comp.region.f <-
  plot.compliance.region('pct.comp.f',
                         ...
                         refline.size = 1) +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.region.bundle <-
  plot.compliance.region('pct.comp.bundle',
                         'ABCDEF Bundle Compliance',
                         ylims = c(0,
                                   max(comp.summary.region.plot[,c('pct.comp.bundle',
                                                                   'pct.perf.bundle')],
                                       na.rm = TRUE)),
                         ybreaks = 0.05)
plot.perf.region.bundle <-
  plot.compliance.region('pct.perf.bundle',
                         ...
                         ylims = c(0,
                                   max(comp.summary.region.plot[,c('pct.comp.bundle',
                                       ...
                                       na.rm = TRUE)),
                         ybreaks = 0.05)
plot.comp.region.elements <-
  plot.compliance.region('pct.elem.comp',
                         'Compliant Bundle Elements',
                         ylims = c(0,
                                   max(comp.summary.region.plot[,c('pct.elem.comp',
                                                                   'pct.elem.perf')],
                                       na.rm = TRUE)),
                         ybreaks = 0.05)
plot.perf.region.elements <-
  plot.compliance.region('pct.elem.perf',
                         'Performed Bundle Elements',
                         ylims = c(0,
                                   max(comp.summary.region.plot[,c('pct.elem.comp',
                                                                   'pct.elem.perf')],
                                       na.rm = TRUE)),
...
compperf.overall <- comp.summary.overall %>%
  dplyr::select(data.month.short, pct.comp.b.sat, pct.perf.b.sat, pct.comp.b.sbt, pct.perf.b.sbt,
                pct.comp.e, pct.perf.e, pct.comp.bundle, pct.perf.bundle) %>%
  gather(key = element_cp, value = pct, pct.comp.b.sat:pct.perf.bundle) %>%
  ...
  mutate(element = factor(ifelse(element == 'sat', 1,
                          ifelse(element == 'sbt', 2,
                          ifelse(element == 'e', 3, 4))),
                          levels = 1:4, labels = c('B: SAT', 'B: SBT',
                                                   'E: Early Mobility', 'ABCDEF Bundle')),
         cp = factor(ifelse(cp == 'comp', 1, 2),
                     levels = 1:2,
                     labels = c('Compliance', 'Performance')))

plot.compperf <-
  ggplot(data = subset(compperf.overall, element != 'ABCDEF Bundle'),
         aes(x = data.month.short, y = pct, group = cp)) +
...
%% -- Create figures for medications, mobility -----------------------------------------------------
<<medmobilityfigure>>=
drug.data <- compliance %>%
  group_by(data.month.short) %>%
...
\section{Compliance by Bundle Element}
The following charts present data for all sites; data specific to community and teaching hospitals nationwide; data by region (WC = West Coast; MW = Midwest; EC = East Coast). Reference lines are
added to indicate ``compliance" for each panel: For overall compliance/performance, the bar is set
at 90\%; for sedation and pain assessments, at 6 assessments per day; and for delirium assessments,
at 2 assessments per day.

\begin{landscape}
<<printcompliancefigs_a, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Element A', fig.height = 6.5, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
...
\noindent \textbf{Assess, prevent and manage pain}: There was documentation that the patient received a minimum of 6 pain assessments in a 24-hour period using a PAD Guideline recommended instrument (i.e., self-report, NRS, BPS or CPOT).

\clearpage
<<printcompliancefigs_b, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Element B', fig.height = 6.5, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
...
\noindent \textbf{Both SAT and SBT}\\
\textbf{Days on sedative}: In patients receiving continuously infused (CI) and/or scheduled intermittent sedatives/opioids, the patient passed a SAT safety screen and received a SAT in the 24 hours period.
\textbf{Days on mechanical ventilation}: In patients receiving invasive mechanical ventilation (MV), the patient passed a SBT safety screen and received a SBT in the 24 hours period.
In patients who received both a SAT and a SBT, the SAT was performed before a SBT in the prior 24 hours period.

\clearpage
<<printcompliancefigs_c, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Element C', fig.height = 6.5, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
...
\noindent \textbf{Choice of analgesia and sedation}: There was documentation that the patient received a minimum of 6 sedation/agitation assessments in the prior 24 hour period using a PAD Guideline recommended instrument (i.e., RASS, SAS).
...
\clearpage
<<printcompliancefigs_d, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Element D', fig.height = 6.5, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
...
\noindent \textbf{Delirium: assess, prevent and manage}: There was documentation that the patient received a minimum of 2 delirium assessments in the prior 24 hour period using a PAD Guideline recommended instrument (i.e., CAM-ICU, ICDSC).

\clearpage
<<printcompliancefigs_e, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Element E', fig.height = 6.5, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
multiplot(plot.perf.e, plot.perf.region.e,
          layout = matrix(1:2, byrow = TRUE, nrow = 2))

@

\noindent \textbf{Early mobility and exercise}: There was documentation that the patient passed an early exercise/mobility safety screen and the patient received exercise/mobility in the prior 24 hours.
...
\clearpage
<<printcompliancefigs_f, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Element F', fig.height = 6.5, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
multiplot(plot.comp.f, plot.comp.region.f,
          layout = matrix(1:2, byrow = TRUE, nrow = 2))

@

\noindent \textbf{Family engagement and empowerment}: Documentation at least once in a 24-hour period that a family member/significant other participated in rounds or a family conference or assisted with the plan of care or the ABCDEF Bundle Care or received education on the bundle elements.

\clearpage
<<printcompperffigs, results = 'asis', fig.cap = 'Compliance and Performance Over Time: Elements B and E, All Sites', fig.height = 5.5, fig.width = 10, fig.pos = 't', fig.align = 'center'>>=
plot.compperf

@

<<printcompliancefigs_overall, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance and Performance Over Time: Total', fig.height = 7, fig.width = 9, fig.pos = 't'>>=
multiplot(plot.comp.bundle, plot.perf.bundle, plot.comp.region.bundle, plot.perf.region.bundle,
...
multiplot(plot.comp.elements, plot.perf.elements,
          plot.comp.region.elements, plot.perf.region.elements,
          layout = matrix(c(1, 2, 1, 2, 3, 4), byrow = TRUE, nrow = 3))
...
hrs.mv.plot.all <- ggplot(data = hrs.mv.plot.data, aes(x = data.month)) +
  ...
  scale_colour_manual(values = as.character(magglass.colors[c(6, 1, 3)]),
                      name = '', guide = FALSE) +
...
when comparing the two cohorts\Sexpr{afterlastmonth.sentence}.)
...
This table presents data shown in Figures \ref{fig:printcompliancefigs_a}-\ref{fig:printcompliancefigs_f} in tabular form. For elements A, C, D, and E, the proportion of days compliant is out of all
days where the patient was in the ICU the full 24 hours (row 1 in table). For element B, the
proportion of SAT compliance is out of all days the patient was in the ICU the full 24 hours and on either continuous or intermittent scheduled sedation; the proportion of SBT compliance, out of all days in the ICU where the patient received invasive mechanical ventilation; and for SAT+SBT, out of all days in the ICU where both an SAT and SBT were performed. For element F, the proportion of compliance is out of all days where a family member or significant other was present.
...
%% -- Figures for April 2017 Meeting Presentation --------------------------------------------------
<<complianceonly>>=
## -- Function to plot compliance percentage over time, all sites only -----------------------------
## Colors, using viridis package, trying to coordinate with SCCM PPT template
comp.color <- as.character(magglass.colors['f'])
perf.color <- as.character(magglass.colors['c'])

plot.compliance.mtg <- function(compvar,
                                ytitle,
                                put.legend = c('bottom', 'top'),
                                refline.size = 1.5){
  ...
  ggplot(data = subset(comp.summary, hosp.type == 'All sites'),
         aes_string(x = 'data.month.short', y = compvar, group = 1)) +
    geom_hline(yintercept = 0.9, linetype = 'dotted', colour = comp.color) +
    geom_vline(xintercept = 1.5, colour = 'white', size = refline.size) +
    geom_line(colour = comp.color) +
    geom_point(colour = comp.color) +
    scale_x_discrete(name = '') +
    scale_y_continuous(name = ytitle,
                       limits = c(0, 1),
                       breaks = seq(0, 1, 0.2),
...
plot.comp.a.mtg <- plot.compliance.mtg('pct.comp.a',
                                       'Pain Assessment Compliance\nOut of All ICU Days') +
  labs(title = 'A: Assess, prevent and manage pain')

plot.comp.b.sat.mtg <- plot.compliance.mtg('pct.comp.b.sat',
                                           'SAT Protocol Compliance\nOut of All ICU Days on Sedation') +
    labs(title = 'B: Both SAT + SBT, SAT Portion')

plot.comp.b.sbt.mtg <- plot.compliance.mtg('pct.comp.b.sbt',
                                           'SBT Protocol Compliance\nOut of All ICU Days on MV') +
  labs(title = 'B: Both SAT + SBT, SBT Portion')

plot.comp.c.mtg <- plot.compliance.mtg('pct.comp.c',
                                       'Sedation Assessment Compliance\nOut of All ICU Days on Sedation') +
  labs(title = 'C: Choice of analgesia and sedation')

plot.comp.d.mtg <- plot.compliance.mtg('pct.comp.d',
                                       'Delirium Assessment Compliance\nOut of All ICU Days') +
  labs(title = 'D: Delirium - assess, prevent and manage')

plot.comp.e.mtg <- plot.compliance.mtg('pct.comp.e',
                                       'Early Mobility Compliance\nOut of All ICU Days') +
  labs(title = 'E: Early mobility and exercise')

plot.comp.f.mtg <- plot.compliance.mtg('pct.comp.f',
                                       'Family Engagement Compliance\nOut of All ICU Days Family Present') +
  labs(title = 'F: Family engagement')

plot.comp.bundle.mtg <- plot.compliance.mtg('pct.comp.bundle',
                                            'Overall ABCDEF Bundle Compliance') +
  labs(title = 'ABCDEF Bundle Compliance')

## Write all compliance figures to PDFs
comp.fig.list <- list(plot.comp.a.mtg, plot.comp.b.sat.mtg, plot.comp.b.sbt.mtg, plot.comp.c.mtg,
                      plot.comp.d.mtg, plot.comp.e.mtg, plot.comp.f.mtg, plot.comp.bundle.mtg)
comp.title.list <- c('comp_a', 'comp_b_sat', 'comp_b_sbt', 'comp_c', 'comp_d', 'comp_e',
                     'comp_f', 'comp_bundle')

mtg_pdfs <- function(ggobj, title){
  pdf(file = paste0('iculib_april2017mtg_', title, '.pdf'), width = 7, height = 5)
  print(ggobj)
  dev.off()
}

map2(.x = comp.fig.list, .y = comp.title.list, mtg_pdfs)

@

<<complianceperformance>>=
## -- Function to plot compliance + performance percentage over time, all sites --------------------
plot.compperf.mtg <- function(el = c("B: SAT", "B: SBT", "E: Early Mobility"),
                              ytitle,
                              put.legend = c('bottom', 'top'),
                              refline.size = 1.5){
  ...
  ggplot(data = subset(compperf.overall, element == el),
         aes(x = data.month.short, y = pct, group = cp, colour = cp)) +
    geom_hline(yintercept = 0.9, linetype = 'dotted', colour = comp.color) +
    geom_vline(xintercept = 1.5, colour = 'white', size = refline.size) +
    geom_line() +
    geom_point() +
    scale_colour_manual(values = c(comp.color, perf.color), name = NULL) +
    scale_x_discrete(name = '') +
    scale_y_continuous(name = ytitle,
                       limits = c(0, 1),
                       breaks = seq(0, 1, 0.2),
...
plot.cp.b.sat.mtg <- plot.compperf.mtg('B: SAT', 'SAT, Out of All ICU Days on Sedation') +
    labs(title = 'B: Both SAT + SBT, SAT Portion')

plot.cp.b.sbt.mtg <- plot.compperf.mtg('B: SBT', 'SBT, Out of All ICU Days on MV') +
  labs(title = 'B: Both SAT + SBT, SBT Portion')

plot.cp.e.mtg <- plot.compperf.mtg('E: Early Mobility', 'Early Mobility, Out of All ICU Days') +
  labs(title = 'E: Early Mobility and Exercise')

plot.cp.bundle.mtg <- plot.compperf.mtg('ABCDEF Bundle',
                                        'Entire ABCDEF Bundle, Out of All Eligible Days') +
  labs(title = 'Entire ABCDEF Bundle')

## Write all compliance + performance figures to PDFs
cp.fig.list <- list(plot.cp.b.sat.mtg, plot.cp.b.sbt.mtg, plot.cp.e.mtg, plot.cp.bundle.mtg)
cp.title.list <- c('cp_b_sat', 'cp_b_sbt', 'cp_e', 'cp_bundle')

map2(.x = cp.fig.list, .y = cp.title.list, mtg_pdfs)
