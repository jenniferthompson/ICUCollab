...
``month" entered in the database and therefore cannot be counted when comparing the two cohorts, and
\Sexpr{afterlastmonth.ids} patients had data collected after month \Sexpr{last.month} which will be
...
compliance <- left_join(compliance,
                        demog[,c('id', 'month', 'data.month', 'data.month.short', 'hospital.type')],
                        by = 'id')
...
comp.summary.overall <- compliance %>%
  ...
  mutate(hospital.type = 'All sites',
         line.alpha = 1)

comp.summary.type <- compliance %>%
  group_by(data.month.short, hospital.type) %>%
  summarise(## Days compliance was tracked
            days.tracked = sum(comp.tracked),
            ## A: Assess, prevent and manage pain
            asmt.pain.median = median(n.painassess.icu, na.rm = TRUE),
            ...
            days.comp.f = sum(comp.f, na.rm = TRUE),
            pct.comp.f = mean(comp.f, na.rm = TRUE)) %>%
  filter(!is.na(data.month.short) & !is.na(hospital.type)) %>%
  mutate(line.alpha = 2)
...
plot.compliance <- function(compvar, ytitle, put.legend = c('bottom', 'top'), refline.size = 1.5){
  ...
  ggplot(data = comp.summary,
         aes_string(x = 'data.month.short', y = compvar,
                    group = 'hospital.type', colour = 'hospital.type', alpha = 'hospital.type')) +
...
#                outlier.shape = NA,
#                alpha = 0.4) +
#   geom_boxplot(aes(colour = hospital.type), fill = NA, alpha = 0.5) +
...
plot.assessments <- function(asmtvar, ytitle){
  ggplot(data = subset(compliance, comp.tracked & !is.na(data.month.short)),
    ...
    # geom_boxplot(aes(colour = hospital.type), fill = NA, alpha = 0.5, outlier.size = 0.5) +
    scale_colour_manual(values = wes_palette('GrandBudapest')[c(2, 1)], guide = 'none') +
    scale_x_discrete(name = '') +
    scale_y_continuous(name = ytitle,
                       breaks = seq(0,
...
box.asmts.a <- plot.assessments(asmtvar = 'n.painassess.icu', ytitle = 'Pain Assessments in 24h')
box.asmts.c <- plot.assessments(asmtvar = 'sed.assess.icu', ytitle = 'Sedation Assessments in 24h')
box.asmts.d <- plot.assessments(asmtvar = 'delirium.assess.icu',
                                ytitle = 'Delirium Assessments in 24h')
...
\begin{landscape}
Table \ref{table:medsmobility} describes sedative and analgesia use, as well as the highest level of
mobility performed, on days patients were in the ICU all 24 hours.

<<printmedmobilitytable, results = 'asis'>>=
latex(summaryM(opioid.icu + benzo.icu + propofol.icu + dex.icu + ketam.icu + antipsyc.typ.icu +
                 antipsyc.atyp.icu + no.sedanal.icu + mobility.high.icu ~ data.month,
      ...
      long = TRUE,
      size = 'small',
      caption = 'Sedatives, Analgesics and Early Mobility in the ICU',
      caption.lot = 'Sedatives, Analgesics and Early Mobility in the ICU',
      label = 'table:medsmobility')
