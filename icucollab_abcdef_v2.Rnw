...
days.mental <- compliance %>%
  ...
  summarise(days.delirium = ifelse(sum(!is.na(delirium.present.valid.f)) == 0, NA,
            ...
            ever.delirious = ifelse(is.na(days.delirium), NA, days.delirium > 0),
            days.delirium.ever = ifelse(!ever.delirious, NA, days.delirium),
            days.coma = ifelse(sum(!is.na(coma.f)) == 0, NA, sum(coma.f == 'Yes', na.rm = TRUE)),
            ever.comatose = ifelse(is.na(days.coma), NA, days.coma > 0),
            days.coma.ever = ifelse(!ever.comatose, NA, days.coma))
...
label(demog$days.delirium.ever) <-
  'Days of delirium per PAD-recommended assessment among those ever delirious'
...
IDs with data recorded up to month \Sexpr{last.month}; of these,
\Sexpr{length(unique(subset(demog, data.time == 'Retrospective')$id))} were collected during the
retrospective period (months 1-6), and
\Sexpr{length(unique(subset(demog, data.time == 'Prospective')$id))} were collected during the
prospective period (months 7-\Sexpr{last.month}). (In addition, \Sexpr{nomonth.ids} patients had no
``month" entered in the database and therefore cannot be counted when comparing the two cohorts;
Month 14 appears to be only 1/3 complete; and \Sexpr{afterlastmonth.ids} patients had data collected
after month \Sexpr{last.month} which will be included in a later report.)
...
\begin{landscape}
\section{ABCDEF Bundle Compliance}
...
results.month <- function(m){
  ...
  c(mdata['days.tracked'],
    npct.a, pain.mediqr,
    mdata['days.sedatives'], npct.b.sat,
    mdata['days.mv'], npct.b.sbt,
...
rownames(compliance.table) <- c('Days compliance was tracked',
                                'Pain assessment compliance',
                                'Pain assessments/day (median/IQR)',
                                'Days on sedatives',
                                'SAT protocol compliance',
                                'Days on mechanical ventilation',
                                'SBT protocol compliance',
                                'Days with SAT + SBT',
                                'Days SAT prior to SBT',
                                'Sedation assessment compliance',
                                'Sedation assessments/day (median/IQR)',
                                'Delirium assessment compliance',
                                'Delirium assessments/day (median/IQR)',
                                'Early mobility protocol compliance',
                                'Days family present',
                                'Family engagement/empowerment compliance')
...
plot.comp.a <- plot.compliance('pct.comp.a', 'Pain Asmt Compliance') +
  labs(title = 'A: Assess, prevent and manage pain') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.b.sat <-
  plot.compliance('pct.comp.b.sat', 'SAT Protocol Compliance',
                  put.legend = 'top', refline.size = 1) +
    labs(title = 'B: Both SAT + SBT') +
    theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.b.sbt <-
  plot.compliance('pct.comp.b.sbt', 'SBT Protocol Compliance',
                  put.legend = 'top', refline.size = 1) +
  labs(title = '')
plot.comp.b.satsbt <- plot.compliance('pct.comp.b.satsbt', 'SAT Prior to SBT',
                                      put.legend = 'top', refline.size = 1) +
  labs(title = '')
plot.comp.c <- plot.compliance('pct.comp.c', 'Sedation Asmt Compliance') +
  labs(title = 'C: Choice of analgesia and sedation') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.d <- plot.compliance('pct.comp.d', 'Delirium Asmt Compliance') +
  labs(title = 'D: Delirium - assess, prevent and manage') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.e <- plot.compliance('pct.comp.e', 'Early Mobility Compliance',
                               put.legend = 'top', refline.size = 1) +
  labs(title = 'E: Early mobility and exercise') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.f <- plot.compliance('pct.comp.f', 'Family Engagement Compliance', refline.size = 1) +
  labs(title = 'F: Family engagement') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))

# ## Quick boxplot to show concept
# compliance.plotdata <- subset(compliance, !is.na(data.month) & comp.tracked)
#
# box.asmts.a <- ggplot(data = compliance.plotdata, aes(x = data.month.short, y = n.painassess.icu)) +
#   geom_boxplot(fill = wes_palette('GrandBudapest')[3],
#                colour = wes_palette('GrandBudapest')[3],
#                outlier.shape = NA,
#                alpha = 0.4) +
#   geom_boxplot(aes(colour = hosp.type), fill = NA, alpha = 0.5) +
#   scale_colour_manual(values = wes_palette('GrandBudapest')[c(2, 1)], guide = 'none') +
#   scale_x_discrete(name = '') +
#   scale_y_continuous(name = 'PAD-Recommended Pain\nAssessments in 24h',
#                      breaks = seq(0,
#                                   max(compliance.plotdata$n.painassess.icu, na.rm = TRUE), 10)) +
#   geom_vline(xintercept = 1.5, colour = 'white', size = 2) +
#   # theme_minimal() +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1),
#         axis.text.y = element_text(hjust = 1))
#
# pdf(file = 'icucollab_boxplot_a.pdf', width = 7, height = 7*0.618)
# box.asmts.a
# dev.off()
...
plot.assessments <- function(asmtvar){
  ggplot(data = subset(compliance, comp.tracked & !is.na(data.month.short)),
    ...
    geom_boxplot(fill = wes_palette('GrandBudapest')[3],
                 ...
                 # outlier.size = 0.5,
                 alpha = 0.4) +
    ## Way too crowded with community + teaching included; can come back to this if V/D/B request
    # geom_boxplot(aes(colour = hosp.type), fill = NA, alpha = 0.5, outlier.size = 0.5) +
    scale_colour_manual(values = wes_palette('GrandBudapest')[c(2, 1)], guide = 'none') +
    scale_x_discrete(name = '') +
    # scale_y_continuous(name = ytitle,
    #                    breaks = seq(0,
    #                                 max(compliance[compliance$comp.tracked, asmtvar], na.rm = TRUE),
    #                                 10)) +
...
box.asmts.a <- plot.assessments(asmtvar = 'n.painassess.icu') +
  scale_y_continuous(name = 'Pain Assessments in 24h', limits = c(0, 35), breaks = seq(0, 35, 5)) +
  geom_hline(yintercept = 6, linetype = 'dotted', colour = ref.line.shade)
box.asmts.c <- plot.assessments(asmtvar = 'sed.assess.icu') +
  scale_y_continuous(name = 'Sedation Assessments in 24h', limits = c(0, 25), breaks = seq(0, 25, 5)) +
  geom_hline(yintercept = 6, linetype = 'dotted', colour = ref.line.shade)
box.asmts.d <- plot.assessments(asmtvar = 'delirium.assess.icu') +
  scale_y_continuous(name = 'Delirium Assessments in 24h', limits = c(0, 10), breaks = seq(0, 10, 2)) +
  geom_hline(yintercept = 2, linetype = 'dotted', colour = ref.line.shade)

## -- Figures by region ----------------------------------------------------------------------------
plot.compliance.region <- function(compvar,
                                   ytitle,
                                   put.legend = c('bottom', 'top'), refline.size = 1.0){
...
plot.comp.region.a <- plot.compliance.region('pct.comp.a', 'Pain Asmt Compliance') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.region.b.sat <- plot.compliance.region('pct.comp.b.sat', 'SAT Protocol Compliance',
                                                 put.legend = 'top', refline.size = 1) +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.region.b.sbt <- plot.compliance.region('pct.comp.b.sbt', 'SBT Protocol Compliance',
                                                 put.legend = 'top', refline.size = 1)
plot.comp.region.b.satsbt <- plot.compliance.region('pct.comp.b.satsbt', 'SAT Prior to SBT',
                                                    put.legend = 'top', refline.size = 1)
plot.comp.region.c <- plot.compliance.region('pct.comp.c', 'Sedation Asmt Compliance') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.region.d <- plot.compliance.region('pct.comp.d', 'Delirium Asmt Compliance') +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.region.e <- plot.compliance.region('pct.comp.e', 'Early Mobility Compliance',
                                             put.legend = 'top', refline.size = 1) +
  theme(plot.title = element_text(hjust = 0, face = 'bold'))
plot.comp.region.f <- plot.compliance.region('pct.comp.f',
...
plot.assessments.region <- function(asmtvar){
  ggplot(data = subset(compliance, comp.tracked & !is.na(data.month.short)),
         aes_string(x = 'data.month.short', y = asmtvar)) +
    facet_wrap(~ hosp.region, nrow = 1) +
    geom_vline(xintercept = 1.5, colour = 'white', size = 0.75) +
    geom_boxplot(fill = wes_palette('GrandBudapest')[3],
                 colour = wes_palette('GrandBudapest')[3],
                 outlier.shape = NA,
                 # outlier.size = 0.5,
...
box.asmts.region.c <- plot.assessments.region(asmtvar = 'sed.assess.icu') +
  scale_y_continuous(name = 'Sedation Assessments in 24h',
                     limits = c(0, 25), breaks = seq(0, 25, 5)) +
  geom_hline(yintercept = 6, linetype = 'dotted', colour = ref.line.shade)
box.asmts.region.d <- plot.assessments.region(asmtvar = 'delirium.assess.icu') +
  scale_y_continuous(name = 'Delirium Assessments in 24h',
...
latex(compliance.table, file = '',
      ...
      where = '!h',
      title = '',
      caption = 'ABCDEF Bundle Compliance',
      caption.lot = 'ABCDEF Bundle Compliance',
      ...
      rgroup = c('',
                 ...
                 'E: Early mobility and exercise',
                 'F: Family engagement and empowerment'),
      n.rgroup = c(1, 2, 6, 2, 2, 1, 2),
      col.just = rep('r', ncol(compliance.table)),
      insert.bottom = 'Note: All data are reported as frequences and percentages unless otherwise noted. SAT = spontaneous awakening trial; SBT = spontaneous breathing trial.')
...
In Figures \ref{fig:printcompliancefigs1}-\ref{fig:printcompliancefigs3}, the overall compliance
...
specific to each region (EC = East Coast; MW = Midwest; WC = West Coast). Reference lines are added
to indicate ``compliance" for each panel: For overall compliance, the bar is set at 90\%; for
sedation and pain assessments, at 6 assessments per day; and for delirium assessments, at 2 assessments per day.

<<printcompliancefigs1, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance Over Time: Elements A \\& B', fig.height = 8, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
multiplot(plot.comp.a, box.asmts.a,
          plot.comp.region.a, box.asmts.region.a,
          plot.comp.b.sat, plot.comp.b.sbt, plot.comp.b.satsbt,
          plot.comp.region.b.sat, plot.comp.region.b.sbt, plot.comp.region.b.satsbt,
          layout = matrix(c(1, 1, 1, 1, 2, 2,
                            1, 1, 1, 1, 2, 2,
                            1, 1, 1, 1, 2, 2,
                            3, 3, 3, 3, 4, 4,
...
<<printcompliancefigs2, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance Over Time: Elements C \\& D', fig.height = 8, fig.width = 10.5, fig.pos = '!h', fig.align = 'center'>>=
...
<<printcompliancefigs3, results = 'asis', fig.cap = 'ABCDEF Bundle Compliance Over Time: Elements E \\& F', fig.height = 4.5, fig.width = 10.5, fig.pos = 't'>>=
multiplot(plot.comp.e, plot.comp.f,
          plot.comp.region.e, plot.comp.region.f,
...
latex(summaryM(mobility.high.icu ~ data.month, data = subset(compliance, comp.tracked)), file = '',
      # landscape = TRUE,
      where = '!h',
      what = '%',
      title = '',
