...
compliance <- left_join(compliance, demog[,c('id', 'data.month', 'hospital.type')], by = 'id')
...
comp.summary.type <- compliance %>%
  group_by(data.month, hospital.type) %>%
  summarise(## Days compliance was tracked
            days.tracked = sum(comp.tracked),
            ## A: Assess, prevent and manage pain
            asmt.pain.median = median(n.painassess.icu, na.rm = TRUE),
            ...
            days.comp.f = sum(comp.f, na.rm = TRUE),
            pct.comp.f = mean(comp.f, na.rm = TRUE)) %>%
  filter(!is.na(data.month) & !is.na(hospital.type)) %>%
  mutate(line.alpha = 2)
...
plot.compliance <- function(compvar, ytitle){
  ggplot(data = comp.summary,
         aes_string(x = 'data.month', y = compvar,
...
plot.comp.a <- plot.compliance('pct.comp.a', 'Pain Assessment Compliance')
plot.comp.b.sat <- plot.compliance('pct.comp.b.sat', 'SAT Protocol Compliance')
plot.comp.b.sbt <- plot.compliance('pct.comp.b.sbt', 'SBT Protocol Compliance')
plot.comp.b.satsbt <- plot.compliance('pct.comp.b.satsbt', 'SAT Prior to SBT')
plot.comp.c <- plot.compliance('pct.comp.c', 'Sedation Assessment Compliance')
plot.comp.d <- plot.compliance('pct.comp.d', 'Delirium Assessment Compliance')
plot.comp.e <- plot.compliance('pct.comp.e', 'Early Mobility Protocol Compliance')
