...
plot.compliance <- function(compvar, ytitle, refline.size = 1.5, dataset = comp.summary.site){
...
plot.comp.a.ref <- plot.compliance('pct.comp.a', 'Pain Asmt Compliance\nOut of All ICU Days',
...
latex(compliance.table, file = '',
      ...
      insert.bottom = '\\emph{Note: All data are reported as frequences and percentages unless otherwise noted. SAT = spontaneous awakening trial; SBT = spontaneous breathing trial.}')

@

In Figures \ref{fig:printcompliancefigs1}-\ref{fig:printcompliancefigs3}, the overall compliance
data shown above is presented visually, alongside reference data for both all ICU Collaborative
sites and this site's peer region/type group (\Sexpr{this.region} \Sexpr{tolower(this.type)}
hospitals). Reference lines are added to indicate ``compliance" for each panel: For overall
compliance/performance, the bar is set at 90\%; for sedation and pain assessments, at 6 assessments
per day; and for delirium assessments, at 2 assessments per day.
...
multiplot(plot.comp.a, plot.comp.a.ref, box.asmts.a,
...
multiplot(plot.comp.c, plot.comp.c.ref, box.asmts.c,
          plot.comp.d, plot.comp.d.ref, box.asmts.d,
...
multiplot(plot.perf.e, plot.comp.f,
          plot.perf.e.ref, plot.comp.f.ref,
...
latex(summaryM(mobility.high.icu ~ data.month, data = subset(compliance.site, comp.tracked)),
      file = '',
      # landscape = TRUE,
      where = '!h',
      what = '%',
